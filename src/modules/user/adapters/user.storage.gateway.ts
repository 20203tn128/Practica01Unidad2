...
export class UserStorageGateway implements IUserRepository{
     async findAll(): Promise<TUser[]> {
        ...
        return userRows.map((user)=><TUser>user); // forma de devolver al usuario devolviendolo de tipo user // no hacerlo en todas las consultas porque no coinciden las propiedades
    }
